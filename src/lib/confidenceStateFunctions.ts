...
export function saveDisplayedConfidenceLevelProceedWarning(
    displayedConfidenceLevelProceedWarning: boolean, setDisplayedConfidenceLevelProceedWarning: (v: boolean) => void) {
    setDisplayedConfidenceLevelProceedWarning(displayedConfidenceLevelProceedWarning)
    const sessionId = getSession()?.id;
    if(sessionId) {
        window.localStorage[buildKey(sessionId)] = `${displayedConfidenceLevelProceedWarning}`
    } else {
