...
export function useSuggestConsultant() {
    ...
    const { setUpdatingSuggestedConsultants, setSuggestedConsultantsError, setConsultantRatings } =
            ...
            fetch(`${reportUrl}/consultant/ratings/${session.id}`)
                .then((response) => {
                    if(response.ok) {
                        ...
                        setSuggestedConsultantsError(t("Failed to fetch suitable consultants."))
                        return null
                    }
                })
                .then(json => {
                    if (json) {
                        const consultantRatings = json["consultant_ratings"] as ConsultantRating[] | undefined
