...
import {isCompletionPopupDisplayed, setCompletionPopupDisplayed} from "../lib/sessionFunctions.ts";
...
export default function useShowCompletionPopup() {
  ...
  useEffect(() => {
    if (messagesOverLowerLimit(messages, messageLowerLimit) && !isCompletionPopupDisplayed()) {
