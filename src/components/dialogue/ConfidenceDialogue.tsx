...
export default function ConfidenceDialogue() {
  ...
  return (
    <GenericDialogue dialogueId={CONFIDENCE_DIALOGUE_ID} clazz="companion-dialogue">
      <DialogueHeader
      ...
      <div className="flex flex-col items-start px-2.5">
        <div className="text-2xl font-bold pt-2">{confidence?.rating ? t(`confidence_${confidence?.rating}`) : "???"}</div>
        <div className="text-base pt-2 mx-auto"><ConfidenceIcon className="w-[5rem] h-[5rem]"/></div>
        <div className="text-base pt-2"><MarkdownComponent content={confidence?.reasoning || ""} /></div>
      </div>
    </GenericDialogue>
  );
};
