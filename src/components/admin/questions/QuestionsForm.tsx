...
import {initialQuestionsConfigState, questionsReducer, QuestionSuggestion} from "./questionsReducer.ts";
...
import {handleError, handleJson, getQuestions, updateQuestion} from "../../../lib/admin/apiClient.ts";
import {supportedLanguages, supportedLanguagesLabels} from "../model/languages.ts";
...
export default function QuestionsForm() {
    ...
    const [state, dispatch] = useReducer(questionsReducer, initialQuestionsConfigState)
    ...
    useEffect(() => {
        getQuestions(reportUrl, state.language)
            ...
            .then((data) => {
                ...
                dispatch({type: "setQuestionSuggestions", questionSuggestions, language: state.language})
            })
            .catch((error) => handleError(error, dispatch));
    }, [state.language]);

    function onSubmit() {
        const questionUpdate = state.questionSuggestions.map(qs => ({id: qs.id, question: qs.question}))
        ...
        updateQuestion(reportUrl, questionUpdate)
            ...
            .then((data) => {
                ...
                if(updated === state.questionSuggestions.length) {
                    dispatch({type: "setMessage", message: t("Updated {{updated}} question(s) successfully", {updated}), messageType: MessageType.SUCCESS})
                } else {
                    dispatch({type: "setMessage", message: t("Some questions could not be updated. Please try again"), messageType: MessageType.FAILURE})
                }
            })
            .catch((error) => handleError(error, dispatch));
    }
    ...
    function isDisabled() {
        return !state.questionSuggestions.every((qs) => qs.question.length > QUESTION_MIN_LENGTH && qs.question.length < QUESTION_MAX_LENGTH)
    }
        ...
        >
            ...
            >
                <Field label={t("Select language")}>
                    <select className="admin-input" onChange={(e) => dispatch({type: "setLanguage", language: e.target.value})}>
                        {supportedLanguages.map((language, i) => (
                            <option key={`lang_${i}`} value={language}>{t(supportedLanguagesLabels[language] ?? language)}</option>
                        ))}
                    </select>
                </Field>
