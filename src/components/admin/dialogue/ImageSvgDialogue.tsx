...
export default function ImageSvgDialogue() {
    ...
    function onSaveSvg(e: React.MouseEvent<HTMLButtonElement>) {
        ...
        if(state.editQuestion && state.editSuggestion) {
            ...
            dispatch({
                type: "saveSuggestionSvg", editQuestion: state.editQuestion, editSuggestion: newSuggestion
            })
    ...
    return (
        ...
        >
            <DialogueHeader onClose={onClose}>
                {editSuggestion && <div dangerouslySetInnerHTML={{__html: editSuggestion}}/>}
            </DialogueHeader>
            <form className="mx-6 my-4">
                {message && <AdminMessage message={message} messageType={MessageType.SUCCESS}/>}
                <textarea className="w-full min-h-64 p-2 my-2 border-2 border-gray-300"
                          value={editSuggestion} onChange={(e) => setEditSuggestion(e.target.value)}/>
                <div className="flex justify-end">
                    <button className="final-report-email w-24 !ml-0 !mr-2" onClick={(e) => {
                        e.preventDefault()
                        onClose()
                    }}>{t("Close")}</button>
