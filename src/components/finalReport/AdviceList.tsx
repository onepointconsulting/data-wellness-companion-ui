...
function AdviceButton({ onClick, label }: { onClick: () => void, label: string }) {
...
function FadeInOutStatusMessage({ statusKey, message }: { statusKey: string, message: string }) {
    const [isVisible, setIsVisible] = useState(true);
    const [displayMessage, setDisplayMessage] = useState(message);
    const previousKeyRef = useRef<string>(statusKey);
    ...
    useEffect(() => {
        ...
        // If key changed (indicating an update), trigger fade out then fade in
        if (previousKeyRef.current !== statusKey) {
            ...
            // Key is the same, just update message without animation
            setDisplayMessage(message);
        }
    ...
    return (
        <div className="flex flex-row justify-end mt-0">
            <span 
                className="text-sm text-gray-500 transition-opacity duration-[400ms] ease-in-out"
...
export function AdviceList({
    ...
    const { deepResearchStatus, deepResearchOutputMap, deepResearchStarted, completedDeepResearchOutput, setDeepResearchStarted, setSelectedDeepResearchOutput, setDeepResearchOutputMap } = useAppStore(useShallow((state) => ({ ...state })));
    ...
    useEffect(() => {
        const currentStatusString = `${deepResearchStatus.status}-${deepResearchStatus.advice}`;
    ...
    }, [deepResearchStatus.status, deepResearchStatus.advice]);
          ...
          fetchDeepResearch(sessionId, reportUrl)
            .then((deepResearchOutputs) => {
              const deepResearchOutputMap: DeepResearchOutputMap =
                deepResearchOutputs.outputs.reduce((a: DeepResearchOutputMap, e: DeepResearchOutput) => (a[e.advice] = e, a), {});
            ...
            {items.map((item, index) => {
                ...
                return (
                    <Fragment key={`markdown-accordion-iterable-${index}`}>
                        ...
                        {handleDeepResearch && socket && !deepResearchStarted && !isCurrentItem && !deepResearchOutputMap[item] && (
                        ...
                        {deepResearchOutputMap[item] &&
                            <AdviceButton onClick={() => {setSelectedDeepResearchOutput(deepResearchOutputMap[item])}} label="View Deep Research" />}
                        {!isCurrentItem && deepResearchStarted && !deepResearchOutputMap[item] &&
                            (
                                <div className="flex flex-row justify-end mt-0">
                                    <span className="text-sm text-gray-500">{t("Please wait...")}</span>
                                </div>
                            )
                        }
                        {isCurrentItem && ["queued", "in_progress"].includes(deepResearchStatus.status) && (
                            <FadeInOutStatusMessage 
                                statusKey={`status-${statusUpdateKey}`}
                                message={t("Generating deep research...", {lastUpdated: new Date().toLocaleString('en-GB')})}
                            />
                        )}
                    </Fragment>
                )
