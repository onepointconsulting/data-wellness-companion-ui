...
export default function ConfidenceIcon({ className }: { className?: string }) {
  ...
  return (
    <div className={`relative w-9 h-9 md:w-11 md:h-11 ${className}`}>
      {updatingConfidence && (<div className="flex flex-row justify-center"><FaHourglassHalf className="w-6 h-6 fill-gray-400" /></div>)}
      {!updatingConfidence && <a href="#" onClick={showConfidenceDialogue}>
        <img
